...
<body>
    ...
    <h1 style="margin-top: 20px;">Register form</h1>
        ...
        if (isset($_POST["submit"])) {
           ...
           $contactDetails = isset($_POST["contactDetails"]) ? $_POST["contactDetails"] : '';
           ...
           if (count($errors) > 0) {
            ...
            $sql = "INSERT INTO users (full_name, email, password,Contact_details) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
            if (mysqli_stmt_prepare($stmt, $sql)) {
                mysqli_stmt_bind_param($stmt, "ssss", $fullName, $email, $passwordHash,$contactDetails);
                mysqli_stmt_execute($stmt);
